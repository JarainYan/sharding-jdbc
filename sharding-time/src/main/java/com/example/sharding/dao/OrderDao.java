...
public interface OrderDao extends JpaRepository<OrderDo,Long>, JpaSpecificationExecutor<OrderDo> {
    ...
    @Modifying
    @Transactional
    @Query(value = " insert into t_order(price,user_id,status,product_id,create_time) value(:price,:userId,:status,:productId,:createTime)",nativeQuery = true)
    int insertOrder(@Param("price") BigDecimal price, @Param("userId")Long userId,
                    @Param("status")String status,@Param("productId")String productId,@Param("createTime") Date createTime);
    ...
    @Query(
            value = "select  * " +
            " from t_order t" +
            " where t.create_time between :statrTime and :endTime",nativeQuery = true)
    List<OrderDo> selectOrderbyIds(@Param("statrTime") Date statrTime,@Param("endTime") Date endTime);

    List<OrderDo> findAllByCreateTimeBetween(@Param("statrTime") Date statrTime,@Param("endTime") Date endTime);
